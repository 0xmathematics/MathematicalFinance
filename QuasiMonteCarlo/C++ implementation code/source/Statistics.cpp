...
double sd_Sample_Mean_True_Expectation(const vector<double>& X, const double X_expectation) {
    double accum = 0.0;
    std::for_each(std::begin(X), std::end(X), [&](const double d) {
        accum += (d - X_expectation) * (d - X_expectation);
        });
    double sd = sqrt(accum / X.size());
    return sd / sqrt(X.size());
}
